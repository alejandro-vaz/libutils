...
pub enum Argument {
    ...
    Alias(Vec<char>),
    Flag(String),
    Setting(String, CliType),
    Target(String),
    Unknown(String)
} impl From<String> for Argument {
    fn from(value: String) -> Self {return match value {
        ref setting if let Some(setting) = setting.strip_prefix("--") && let Some((key, value)) = setting.split_once('=') => Argument::Setting(key.to_string(), CliType::from(value.to_string())),
